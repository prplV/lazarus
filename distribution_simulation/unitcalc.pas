...
begin
  ...
  if libhandle <>0 then begin
     ShowMessage('positive if');
     add:=TAdd(GetProcedureAddress(libhandle, 'Add'));
     ShowMessage('positive add :=');
     if Assigned(Add) then begin
             ShowMessage('positive 2nd if');
             z:=Add(StrToInt(Edit1.Text), StrToInt(Edit2.Text));
             ShowMessage('positive z :=');
             ModalResult := z;
             ShowMessage('positive modalresult :=');
     end;
     FreeLibrary(libhandle);
     ShowMessage('positive free');
  end;
end;
